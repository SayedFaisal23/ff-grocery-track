...
>
    ...
    @if($isPurchaseRequest)
        <dl class="claim-detail-rows">
            ...
            @if($isDirectorCreditCardPayment)
                <div class="claim-detail-row">
    ...
    @if($isPurchaseRequest && $claim->payment_proof_attachment && ($isSuperadmin || $isOwner))
        <div class="claim-attachment-link">
            <a
                href="{{ route('tuntutan.payment-proof', $claim) }}"
                target="_blank"
                rel="noopener"
                class="btn btn-secondary btn-sm {{ $paymentProofAwaitingView ? 'claim-document-awaiting-view' : '' }}"
                data-attachment-open-link
            >
    ...
    @if($claim->canUploadAttachment() && $isOwner)
        <form
    ...
    @if($isSuperadmin && $claim->canUploadPaymentProof())
        <form
